class Casacore < Formula
  ...
  url "https://github.com/casacore/casacore/archive/v3.6.1.tar.gz"
  ...
  option "with-test", "Build c++ tests (broken on 3.5.0)"

  depends_on "cmake" => :build

  depends_on "casacore-data" => :recommended
  depends_on "cfitsio"
  depends_on "fftw"
  ...
  def install
    ...
    mkdir "build/#{build_type}" do
      ...
      cmake_args << "-DBUILD_TESTING=#{(build.with? "test") ? "ON" : "OFF"}"
